...
const toYaml = (obj, indent = 0) => {
  ...
  return Object.entries(obj).reduce((result, [key, value]) => {
    ...
    if (typeof value === "object")
      return result + `${padding}${key}:\n${toYaml(value, indent + 2)}`;
    return (
      result +
      `${padding}${key}: ${key === "name" ? `'${String(value).replace(/'/g, "''")}'` : value}\n`
    );
  }, "");
};
...
const getStreamSettings = (type, params) => {
  ...
  const bool = (val) =>
    val === "true" || val === true || val === "1" || undefined;
  ...
  if (type === "tcp" && params.headerType)
    output.tcpSettings = { header: { type: params.headerType } };
  if (type === "raw" && params.headerType)
    output.rawSettings = { header: { type: params.headerType } };
  if (type === "xhttp") {
    let extra;
    try {
      extra = params.extra
        ? JSON.parse(decodeURIComponent(params.extra))
        : undefined;
    } catch {}
...
function convertToMihomoYaml(proxyConfig) {
  ...
  if (streamSettings.network === "ws")
    common["ws-opts"] = {
      ...
      headers: streamSettings.wsSettings?.host
    ...
    common["http-upgrade-opts"] = {
      ...
      headers: streamSettings.httpupgradeSettings?.host
...
function generateConfigForCore(uri, core = "xray", existingConfig = "") {
  ...
  if (core === "mihomo" && uri.includes("type=xhttp"))
    throw new Error("XHTTP в Mihomo не поддерживается");
  if (core !== "mihomo" && uri.startsWith("http"))
    throw new Error("Подписки в Xray не поддерживаются");
  if (
    core !== "mihomo" &&
    (uri.startsWith("hysteria2") || uri.startsWith("hy2"))
  )
    throw new Error("Hysteria2 в Xray не поддерживается");

  const config = parseProxyUri(uri);
  if (config.tag === "PROXY" || existingConfig.includes(config.tag))
    config.tag = generateName(config.protocol);
