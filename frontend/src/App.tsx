...
function AppContent() {
  ...
  async function checkVersion() {
    try {
      ...
      if (data.success && data.version) {
        dispatch({ type: "SET_VERSION", version: data.version.replace(/^v/i, ""), isOutdatedUI: !!data.outdated?.ui })
        if (data.show_toast?.ui) showToast("Доступна новая версия XKeen UI")
        if (data.show_toast?.core) showToast(`Доступна новая версия ${capitalize(state.currentCore)}`)
      }
    } catch {
      /* ignore */
    }
  ...
  async function loadConfigs(core?: string) {
    ...
    try {
      ...
      if (result.success && result.configs) {
        ...
        const yamlConfig = configs.find((c: any) => c.filename === "config.yaml")
  ...
  async function handleSaveAndSwitch() {
    ...
    if (config && editorRef.current) {
      ...
      await apiCall<any>("PUT", "configs", { action: "save", filename: config.filename, content })
  ...
  function handleDiscardAndSwitch() {
    ...
    if (pendingSwitchIndex < 0) return
    const config = configs[activeConfigIndex]
    if (config && editorRef.current) editorRef.current.setValue(config.savedContent)
    dispatch({ type: "SAVE_CONFIG", index: activeConfigIndex, content: config.savedContent })
    dispatch({ type: "SHOW_MODAL", modal: "showDirtyModal", show: false })
    applySwitch(pendingSwitchIndex, configs)
  }
  ...
  async function importTemplate(url: string) {
    ...
    if (editorRef.current) {
      ...
      dispatch({ type: "UPDATE_CONFIG_DIRTY", index: state.activeConfigIndex, isDirty: true, content })
    }
  ...
  function addToConfig(generated: string, type: string, position: "start" | "end") {
    ...
    if (core === "mihomo") {
      ...
      if (position === "start") {
        ...
        const insertOffset = nextKey === -1 ? current.length : afterMarker + nextKey
        insertAtOffset(insertOffset, generated + "\n")
      }
    } else {
      try {
        const obj = JSON.parse(current)
        const outbound = JSON.parse(generated)
        if (!Array.isArray(obj.outbounds)) obj.outbounds = []
        const insertIndex = position === "start" ? 0 : obj.outbounds.length
        if (position === "start") obj.outbounds.unshift(outbound)
        else obj.outbounds.push(outbound)
        const newContent = JSON.stringify(obj, null, 2)
        // Find line of the inserted outbound to scroll to it
        const tempLines = newContent.split("\n")
        let outboundsLine = tempLines.findIndex((l) => l.includes('"outbounds"'))
        let targetLine = outboundsLine + 1 + insertIndex * (JSON.stringify(outbound, null, 2).split("\n").length + 1)
        monacoEditor.executeEdits("add-to-config", [
          {
        ...
        monacoEditor.revealLineInCenter(Math.max(1, targetLine))
      } catch (e: any) {
  ...
  return (
    <div className="min-h-dvh flex flex-col bg-background">
      ...
      <ToastContainer />
      <DirtyModal onSaveAndSwitch={handleSaveAndSwitch} onDiscardAndSwitch={handleDiscardAndSwitch} />
