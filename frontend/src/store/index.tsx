...
function reducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    case 'SET_SERVICE_STATUS':
      return { ...state, serviceStatus: action.status, pendingText: action.pendingText ?? state.pendingText }

    case 'SET_CORE_INFO':
      return { ...state, currentCore: action.currentCore, coreVersions: action.coreVersions, availableCores: action.availableCores }
    ...
    case 'UPDATE_CONFIG_DIRTY': {
      ...
      configs[action.index] = { ...configs[action.index], isDirty: action.isDirty, ...(action.content !== undefined ? { content: action.content } : {}) }
      return { ...state, configs }
    }

    case 'SAVE_CONFIG': {
      const configs = [...state.configs]
      configs[action.index] = { ...configs[action.index], content: action.content, savedContent: action.content, isDirty: false }
      return { ...state, configs }
    }
    ...
    case 'SET_VERSION':
      return { ...state, version: action.version, isOutdatedUI: action.isOutdatedUI }
...
interface AppContextValue {
  ...
  showToast: (message: string | { title: string; body: string }, type?: 'success' | 'error') => void
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const showToast = useCallback((message: string | { title: string; body: string }, type: 'success' | 'error' = 'success') => {
