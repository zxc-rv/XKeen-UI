...
const SwitchRow = ({ id, label, checked, onToggle }: { id: string; label: string; checked: boolean; onToggle: (v: boolean) => void }) => (
...
export function SettingsModal() {
  ...
  return (
    <Dialog open={state.showSettingsModal} onOpenChange={(open) => !open && close()}>
      <DialogContent className="max-w-lg! flex flex-col overflow-hidden" style={{ maxHeight: "80vh" }}>
        ...
        <ScrollArea className="flex-1" style={{ maxHeight: "calc(80vh - 140px)" }}>
            ...
            {activeTab === "gui" && (
              <div>
                <div className="mb-3 p-3 rounded-lg bg-yellow-700/15 border border-amber-500/40 text-xs tracking-wide text-amber-400 flex items-start gap-2">
                  ...
                  <span>
                    Функция экспериментальная. Сделайте бэкап конфигурация перед включением. Не совместимо с комментариями в конфиге.
                  </span>
                </div>
                <SwitchRow
                  id="gui-routing"
                ...
                />
                <Separator />
                <SwitchRow id="gui-log" label="Log" checked={settings.guiLog} onToggle={(v) => toggle("guiLog", "gui.log", v)} />
                <Separator />
                <SwitchRow
                  id="auto-apply"
            ...
            {activeTab === "updates" && (
              <div>
                <SwitchRow
                  ...
                  onToggle={(v) => toggle("autoCheckUI", "updater.auto_check_ui", v)}
                />
                <Separator />
                <SwitchRow
                  id="auto-core"
                  ...
                  onToggle={(v) => toggle("autoCheckCore", "updater.auto_check_core", v)}
                />
                <Separator />
                <SwitchRow
                  id="backup"
                  ...
                  onToggle={(v) => toggle("backupCore", "updater.backup_core", v)}
                />
                <Separator />
                ...
                <Select value={String(settings.timezone)} onValueChange={setTimezone}>
                  ...
                  <SelectContent>
                    {Array.from({ length: 27 }, (_, i) => i - 12).map((offset) => (
                      <SelectItem key={offset} value={String(offset)} className="p-2 text-sm">
                        UTC{offset >= 0 ? "+" : ""}
                        {offset}
                      </SelectItem>
                    ))}
