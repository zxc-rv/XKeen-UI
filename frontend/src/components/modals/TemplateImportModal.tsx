...
export function TemplateModal({ onImport }: { onImport: (url: string) => Promise<void> }) {
  ...
  return (
    <Dialog open={state.showTemplateModal} onOpenChange={(open) => !open && close()}>
      <DialogContent className="max-w-140! max-h-[80vh] flex flex-col overflow-hidden">
        <DialogHeader className="shrink-0">
          ...
          <DialogDescription className="flex items-center justify-between w-full">
            <span>
              Выберите готовый шаблон конфигурации для <span className="text-chart-2 font-medium">{capitalize(currentCore)}</span>
            </span>
                ...
                <div
                  ...
                  className={cn(
                    ...
                    selectedUrl === template.url
                      ? "border-chart-2 bg-primary/15"
                      : "border-border hover:bg-primary/15 hover:border-chart-2",
                  )}
                >
        ...
        <DialogFooter className="shrink-0">
          <Button onClick={handleImport} disabled={!selectedUrl || importing} className="h-9 w-full">
