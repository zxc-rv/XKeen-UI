...
const ConnectionRow = memo(function ConnectionRow({
  ...
  return (
    <TableRow className="hover:bg-muted/50 cursor-pointer transition-colors" onClick={() => onSelect(conn)}>
      <TableCell className="max-w-[30rem] pl-3 text-[13px] md:max-w-40">
        <Tooltip>
          <TooltipTrigger asChild>
            <div
              role="button"
              tabIndex={0}
              className="flex w-fit max-w-full cursor-pointer items-center gap-1 overflow-hidden text-left hover:opacity-80"
              onClick={(e) => {
                e.stopPropagation()
                if (first) onApplyFilter(first)
              }}
              onKeyDown={(e) => {
    ...
    <TooltipProvider delayDuration={700} skipDelayDuration={0}>
      <div className="border-border bg-input-background absolute inset-4 flex flex-col overflow-hidden rounded-xl border">
        ...
        <div className="flex-1 overflow-auto [scrollbar-width:thin]">
          <Table className="min-w-[60rem] md:min-w-190">
