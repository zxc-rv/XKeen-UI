...
const RULE_FIELDS = {
  domain: { type: "array" as const, placeholder: "youtube.com, geosite:youtube" },
  ...
  network: { type: "buttons" as const, options: ["tcp", "udp"], isString: true },
  inboundTag: { type: "buttons" as const },
  protocol: { type: "buttons" as const, options: ["http", "tls", "quic", "bittorrent"] },
}
...
export function RoutingPanel({ editorRef, configs, activeConfigIndex }: Props) {
  ...
  function loadAvailable() {
    let outbounds: string[] = [],
      ...
      balancers: string[] = []
    try {
      const c = configs.find((x) => x.filename.toLowerCase().includes("outbound"))
      if (c) {
    ...
    try {
      const c = configs.find((x) => x.filename.toLowerCase().includes("inbound"))
      if (c) {
        const j = JSON.parse(stripComments(c.content))
    ...
    setAvailable({ outbounds: [...new Set(outbounds)], inbounds: [...new Set(inbounds)], balancers: [...new Set(balancers)] })
  }
      ...
      try {
        ...
        if (triggerSoftRestart && state.settings.autoApply && state.serviceStatus === "running") {
          ...
          if (activeConfig) {
            ...
            await apiCall<any>("PUT", "configs", { action: "save", filename: activeConfig.filename, content })
            ...
            const r = await apiCall<any>("POST", "control", { action: "softRestart", core: state.currentCore })
...
const RuleCard = forwardRef<HTMLDivElement, RuleCardProps>(function RuleCard(
  ...
  const conditionFields = Object.keys(rule).filter((k) => !["outboundTag", "balancerTag", "ruleTag"].includes(k))
  ...
  function addField(f: FieldName) {
    ...
    onUpdate({ ...rule, [f]: cfg.type === "array" || cfg.type === "buttons" ? [] : "" })
  }
                  ...
                  {((fieldName === "inboundTag" ? available.inbounds : (cfg as any).options) ?? []).map((opt: string) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn("cursor-pointer px-3 py-0.75 rounded text-xs border transition-colors", active ? c.a : c.i)}
                      >
        ...
        <Select value={outboundValue} onValueChange={changeOutboundValue}>
          ...
          <SelectContent className="p-1.5">
            ...
            {(isBalancer ? available.balancers : available.outbounds).length === 0 && (
              <div className="px-3 py-2 text-xs text-muted-foreground">{isBalancer ? "Балансиры не найдены" : "Аутбаунды не найдены"}</div>
            )}
          </SelectContent>
        </Select>
      </div>
