...
use std::{env, path::Path, sync::{Arc, RwLock}};
...
async fn main() {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "-d" => _debug = true,
            "-v" | "-V" => { println!("XKeen UI {} ({}/{})", VERSION, std::env::consts::OS, get_arch()); std::process::exit(0); }
            _ => {}
        }
    }
...
fn load_settings() -> AppSettings {
    let mut s = std::fs::read_to_string(APP_CONFIG).and_then(|c| serde_json::from_str(&c).map_err(|e| e.into())).unwrap_or_else(|_| AppSettings::default());
