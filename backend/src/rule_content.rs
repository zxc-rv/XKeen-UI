...
pub async fn get_rule_provider_content(
    ...
    let docs = match YamlLoader::load_from_str(&config) {
        Ok(v) => v,
        Err(e) => return error_response(format!("YAML хуёвый, ошибка парсинга: {e}")),
    };
    let Some(parsed) = docs.first() else {
        return error_response("YAML пустой, читать нечего".into());
    };

    let provider = &parsed["rule-providers"][params.name.as_str()];
    if provider.is_badvalue() {
        return error_response(format!("Провайдер '{}' проёбан (не найден)", params.name));
    }
        ...
        .is_some_and(|v| v.eq_ignore_ascii_case("inline"))
    {
        ...
        if items.is_empty() {
            return error_response("Payload пуст или тупо не найден".into());
        }
