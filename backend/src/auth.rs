...
use crate::types::{ApiResponse, AppState, APP_CONFIG};
...
fn get_client_ip(headers: &HeaderMap, addr: SocketAddr) -> String {
    headers.get("x-real-ip").or_else(|| headers.get("x-forwarded-for"))
...
fn get_session_cookie<'a>(headers: &'a HeaderMap) -> Option<&'a str> {
    headers.get("cookie")?.to_str().ok()?.split(';').find_map(|pair| {
        pair.trim().strip_prefix("session_id=")
    })
}
...
fn set_cookie_header(headers_in: &HeaderMap, value: String, max_age: u64) -> HeaderMap {
    ...
    let secure = if headers_in.get("x-forwarded-proto").and_then(|v| v.to_str().ok()) == Some("https") { "Secure; " } else { "" };
    let cookie = if max_age > 0 {
        format!("{}={}; HttpOnly; {}SameSite=Strict; Path=/; Max-Age={}", SESSION_COOKIE, value, secure, max_age)
    } else {
        format!("{}={}; HttpOnly; {}SameSite=Strict; Path=/", SESSION_COOKIE, value, secure)
    };
...
pub async fn get_login_info(State(state): State<AppState>, headers: HeaderMap) -> impl IntoResponse {
    ...
    let authenticated = get_session_cookie(&headers)
        .map_or(false, |cookie| is_session_valid(&s.auth.session_ids, cookie));
...
pub async fn post_setup(State(state): State<AppState>, headers: HeaderMap, Json(req): Json<PasswordReq>) -> impl IntoResponse {
    if state.settings.read().unwrap().auth.password_hash.is_some() {
        return (StatusCode::FORBIDDEN, Json(ApiResponse::<()> { success: false, error: Some("Password already set".into()), data: None })).into_response();
    }

    let password = req.password;
    let hash = tokio::task::spawn_blocking(move || hash_password(&password)).await.unwrap();
    ...
    (set_cookie_header(&headers, session_id, 0), Json(ApiResponse::<()> { success: true, error: None, data: None })).into_response()
}
...
pub async fn post_login(
    ...
    let ip = get_client_ip(&headers, addr);
    {
        ...
        if entry.0 >= MAX_ATTEMPTS {
            ...
            return (StatusCode::TOO_MANY_REQUESTS, Json(ApiResponse::<()> {
                success: false,
                error: Some(format!("Слишком много попыток. Повторите через {} секунд", LOCKOUT_SECS)),
                data: None,
            })).into_response();
        }
    }
    ...
    let hash = match state.settings.read().unwrap().auth.password_hash.clone() {
        Some(h) => h,
        None => return (StatusCode::FORBIDDEN, Json(ApiResponse::<()> { success: false, error: Some("No password set".into()), data: None })).into_response(),
    };

    let password = req.password;
    let is_valid = tokio::task::spawn_blocking(move || verify_password(&password, &hash)).await.unwrap_or(false);
    ...
    if !is_valid {
        ...
        entry.0 += 1;
        entry.1 = Instant::now();
        println!("{} [WARN] Incorrect password attempt [{}] ({}/{})", crate::logger::ts(), ip, entry.0, MAX_ATTEMPTS);
        return (StatusCode::UNAUTHORIZED, Json(ApiResponse::<()> { success: false, error: Some("Неверный пароль".into()), data: None })).into_response();
    }
    ...
    update_auth(&state, |auth| { auth.session_ids.push(session_val); }).await;

    (set_cookie_header(&headers, session_id, max_age), Json(ApiResponse::<()> { success: true, error: None, data: None })).into_response()
}
...
pub async fn post_logout(State(state): State<AppState>, headers: HeaderMap) -> impl IntoResponse {
    if let Some(cookie) = get_session_cookie(&headers) {
        ...
        update_auth(&state, |auth| auth.session_ids.retain(|id| !id.starts_with(&cookie) && id != &cookie)).await;
    }
    (clear_cookie_header(), Json(ApiResponse::<()> { success: true, error: None, data: None }))
}
...
async fn save_auth_to_config(state: &AppState) {
    ...
    let mut file_json: serde_json::Value = tokio::fs::read_to_string(APP_CONFIG).await
        .ok().and_then(|c| serde_json::from_str(&c).ok())
        .unwrap_or(serde_json::json!({}));
    file_json["auth"] = serde_json::to_value(auth).unwrap();
    let _ = tokio::fs::write(APP_CONFIG, serde_json::to_string_pretty(&file_json).unwrap()).await;
}